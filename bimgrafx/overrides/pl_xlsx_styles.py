...
def get_pl_row_outline_levels(metadata: XLSXMetadata) -> dict:
	levels = {}
...
def get_profit_and_loss_xlsx_styles(metadata: XLSXMetadata) -> dict:
	...
	for row_idx, row in metadata.row_map.items():
		...
		clean_account = account.strip("'\"").strip()
		is_group = bool(row.get("is_group"))
		is_total = clean_account.lower().startswith("total ")
		is_profit = clean_account.lower() in {
	...
	result["row_outline_levels"] = get_pl_row_outline_levels(metadata)
	return result